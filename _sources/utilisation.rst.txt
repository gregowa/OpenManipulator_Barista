...
2. Reconstruire l'espace de travail depuis le dossier catkin_ws :  catkin\_make \\ clean
    catkin_make} 
3. item Source devel/setup.bash : recharge l'espace de travail et permet d'éxecuter la commande (à executer depuis catkin_ws, à faire à chaque fois que l'on lance un nouveau noeud ROS).
4. Ouvrir un nouveau terminal et lancer le contrôleur de l'OpenManipulator} : roslaunch open_manipulator_controller open_manipulator_controller.launch . A l'exécution de cette commande, les moteurs devraient se raidir et des informations devraient défiler dans le terminal. 
...
5. Ouvrir un nouveau terminal et lancer la télécommande pour piloter l'OpenManipulator : roslaunch open_manipulator_teleop open_manipulator_teleop_keyboard.launch}
